# Generated with Claude Code (claude-sonnet-4-6). Test design directed by David Huang.
...
def _fill(buf: DictReplayBuffer, n: int) -> None:
    for i in range(n):
        buf.push(_make_obs(i), action=i % 32, reward=float(i), next_obs=_make_obs(i + 1000), done=(i % 10 == 9))
...
def test_rewards_roundtrip():
    ...
    for i in range(10):
        small.push(_make_obs(i), action=0, reward=float(i * 10), next_obs=_make_obs(i), done=False)
...
def test_circular_overwrite(buf):
    ...
    for i in range(10):
        buf.push(_make_obs(200 + i), action=0, reward=999.0, next_obs=_make_obs(300 + i), done=False)
