...
def obs_to_tensor(obs: Dict, device: torch.device) -> Dict[str, torch.Tensor]:
    """Convert a numpy Dict obs to tensors. Handles both single (no batch dim) and batched obs."""
    ...
    return {
        ...
        "next_fruit":    torch.tensor(nxt_np.astype(np.int64),     dtype=torch.long,    device=device),
    }
